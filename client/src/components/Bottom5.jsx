...
function Bottom5(props) {
  return (
    <>
    {/* <h4>Bottom 5 Crops in this month</h4> */}
      ...
      <tbody className='rounded'>
        {props.data.map((row, index) => (
          <tr key={index} className={index % 2 === 0 ? 'bg-slate-100' : 'bg-slate-300'}>
            ...
            <td className="p-2 border-t border-gray font-mono text-sm text-black whitespace-no-wrap"><p>{row[0]}</p></td>
            <td className="p-2 border-t border-gray font-mono text-sm text-black whitespace-no-wrap"><p>{row[1]}</p></td>
            <td className="p-2 border-t border-gray font-mono text-sm text-black whitespace"><div className='flex align-middle justify-center'><p className='text-md  pt-3'>{row[2]}%</p>{" "}
